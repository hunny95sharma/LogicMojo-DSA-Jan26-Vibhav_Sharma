...
public class LongestSubstringNonRepeating {
    ...
    for (int i = 0; i < input.length(); i++) {
        ...
        int j = i;
        int length = nonRepeatingSubstring(input, i, input.length() - 1);
	...
	static int[] longestSubstringNonRepeatingSlidingWindowForAllCharacters(String input) {
		int start=0;
		int startIndex=-1;
		int endIndex=-1;
		int maxLength=0;
	...
	static int []longestSubstringNonRepeatingSlidingWindowForOnlyAlphabets(String s){
		int start=0;
		int end=0;
		int maxLength=0;
		int startIndex=-1;
		...
		while(end<s.length()) {
			if(s.charAt(end)!=' ') {
			freqCharArray[s.charAt(end)-'a']=freqCharArray[s.charAt(end)-'a']+1;
			while(freqCharArray[s.charAt(end)-'a']>1) {
				freqCharArray[s.charAt(start)-'a']=freqCharArray[s.charAt(start)-'a']-1;
				start++;
			}
			if((end-start+1)>maxLength) {
				maxLength=end-start+1;
